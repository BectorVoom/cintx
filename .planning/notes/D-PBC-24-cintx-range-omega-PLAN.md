...
status: |
  cintx stages 0-3 LANDED 2026-08-30; stage 4 (device omega arms) NOT STARTED —
  range separation routes to the host engine, explicitly and logged; stage 5 is
  consumer work in pyscf_rs and is now unblocked. See §9.
autonomous: false
must_haves:
  truths:
...
## 9. Implementation record — cintx, 2026-08-30

Everything in this section is in the cintx tree. Stage 5 is untouched: it is
`pyscf_rs` work, it pins a released cintx, and it moves a committed reference
energy — see "What stage 5 inherits" at the end.

### Stage 0 — measured first ✅

`crates/cintx-oracle/tests/range_omega_parity.rs`, and the committed record at
`.planning/notes/D-PBC-24-range-omega-sweep.out` (regenerate with
`CINTX_RANGE_OMEGA_OUT=<path> CINTX_ORACLE_BUILD_VENDOR=1 cargo test -p
cintx-oracle --features cpu --test range_omega_parity record_the_omega`).

Ten shell tuples spanning `rys_order` 1..5 across `int2e` / `int3c2e` /
`int2c2e`, swept over ω ∈ {0, +0.3, +0.8, −0.3, −0.8}, with the SR regime named
per tuple and `SR + LR − full` recorded alongside. libcint's own residuals on
that identity are 0 to 4.4e-16.

One correction to §3.4's table: the fixture has to be genuinely multi-centre.
A same-centre `(s,s|d)` is identically zero by angular symmetry, so it would
have passed every check while asserting nothing.

### Stage 1 — the control plane ✅

`ExecutionOptions::range_omega`, `OperatorEnvParams::range_omega`,
`SessionBuilder::with_range_omega`, propagation on both the safe path
(`cintx-rs/src/api.rs`) and the raw path (`cintx-compat/src/raw.rs`, which now
exports `PTR_RANGE_OMEGA` rather than only naming it in a warning),
`validate_range_omega_value` / `_env_params`, and a new
`cintx-runtime/src/range_omega.rs` holding the sign convention, `EXPCUTOFF_SR`,
the `nrys_roots` doubling rule and the operator-support predicate.

Three deviations from §4's wording, all deliberate:

1. **`estimate_workspace_request` takes `range_omega: Option<f64>`, not
   `&ExecutionOptions`.** `ExecutionPlan::new` has to re-derive the same request
   to check it, and it does not have the options — it has the `WorkspaceQuery`.
   Threading the omega through the query (`WorkspaceQuery::range_omega`,
   `WorkspaceRequest::rys_roots`) makes query and evaluate agree *by
   construction* and lets `planning_matches` treat a changed ω as the contract
   drift it is. Passing `&ExecutionOptions` could not have done either.
2. **The SR doubling is charged as a delta, not as a new baseline term.** cintx's
   workspace request is `output + basis + shell` bytes and has never had a Rys
   term; adding one unconditionally would have moved every existing request. Only
   the *extra* roots are charged, so `range_omega = None` is byte-identical.
3. **ω is accepted only on `int2e`, `int3c2e`, `int2c2e`** — the scalar rows,
   `operator_name == "electron-repulsion"`. §4.6 asked for rejection on the
   families with no `1/r₁₂` kernel; this is stricter, and rejects the `ip1`/
   `ipip1`/… derivative rows of the 2e families too. Those DO read `env[8]`
   upstream (`g2e.c:171` shares `CINTg0_2e` with the whole `int2e_*` space), so
   ignoring ω there would be exactly the silent-substitution failure this
   document exists to prevent. Widening it later is a kernel change plus a
   `supports_range_omega` line, not a redesign.

### Stage 2 — LR, and SR for `rys_order <= 3` ✅

`crates/cintx-cubecl/src/math/range_separation.rs` is the single port of
`CINTg0_2e`'s omega branch; `fill_g_tensor_2e` (via `fill_g_tensor_2e_range`),
`fill_g_tensor_3c2e` and `fill_g_tensor_2c2e` all call it. `build_2e_shape_omega`
applies the root doubling to the stride metadata, which is where it has to
happen. `EXPCUTOFF_SR` is ported; the `theta*x > cutoff` half of the early return
is not, for the reason §3.5 gives — cintx implements no `cceij` screening for the
full-range operator either, so skipping it keeps more primitives, which is the
conservative direction.

§4 assumed all three families had a host arm to extend. `int2c2e` and `int2e`
did (their `nroots > device_ceiling` fallbacks). **`int3c2e` did not** — its
scalar path is device-only and returned `UnsupportedApi` above the ceiling — so
`host_3c2e_cart_blocks` was added, built from the `fill_g_tensor_3c2e` →
`split_ij_hrr` → `contract_3c2e` chain that `scalar_device_tests` already proves
device-equivalent, and emitting `run_3c2e_device`'s exact block layout so the
whole post-processing tail is unchanged.

### Stage 3 — `CINTsr_rys_roots` ✅ (and it was much smaller than 1-2 weeks)

The estimate assumed porting a quadrature family. In fact `lower` enters
libcint in exactly ONE place — the FMT moments — and cintx already had every
downstream stage: `rdk_rys_roots`, `r_lsmit_dd`, `cint_polynomial_roots`,
`rys_wheeler_partial`, `lrys_wheeler_partial_dd`, the `Dd` arithmetic. What was
missing was `fmt_erfc_like` / `fmt1_erfc_like` and their dd images, the
`lower != 0` Jacobi and Laguerre moments, `segment_solve` / `segment_solve1`, and
the dispatch. All of it is one section at the bottom of `rys_wheeler.rs`,
entered from `range_separation.rs`'s `rys_order == nroots` arm.

The vendored oracle build has quadmath disabled, so `CINTqrys_*` IS `CINTlrys_*`
(`rys_roots.h:36-38`) — which is what cintx's double-double models — and
`WITH_POLYNOMIAL_FIT` is not defined, so `CINTsr_rys_polyfits` is not part of the
dispatch being reproduced.

**One real finding.** `roots_jacobi_data` stores libcint's `lJACOBI_COEF` /
`ALPHA` / `BETA` — written out to ~34 digits as `long double` — truncated to
`f64`. Invisible at `lower == 0`; as `lower → 1` the `∫_lower^1` moment sum
cancels catastrophically and the *coefficient* precision becomes the accuracy
floor of the whole quadrature, leaving cintx ~1000x behind the 80-bit reference.
`math::roots_jacobi_dd_data` carries the same literals as double-double pairs
(~32 digits, ahead of the reference rather than behind it), worth 10³–10⁶ at
`nroots >= 8`.

**And one limit worth naming.** Past roughly `nroots >= 8` with `lower > 0.99`,
libcint's own `CINTsr_rys_roots` breaks down: zero-padded weights, negative roots
below the `lower²/(1−lower²)` floor, `err == 0` regardless. Verified against a
60-digit mpmath Gauss rule at `(nroots, x, lower) = (12, 11, 0.999)`, where the
true nodes are all in `[505, 4.1e4]`. `sr_rys_roots_parity.rs` skips those points
through an explicit `reference_is_a_valid_gauss_rule` predicate with a pinned
skip count, rather than absorbing them into a loose tolerance. It does not reach
the integrals: the affected roots carry negligible weight.

### Stage 4 — device arms ❌ NOT STARTED, and routing is explicit

`launch_two_electron_typed`, `launch_center_3c2e_typed` and
`launch_center_2c2e_typed` each compute
`route_host = is_range_separated(range_omega)` and `tracing::debug!` it with the
family, ω, `rys_order` and `nroots`. The device `#[cube]` kernels are untouched:
their comptime `nroots` arms evaluate `rys_root{1..5}` at a single argument,
while SR needs two evaluations plus a root rescaling, and its doubled
`nroots = 6` at `rys_order = 3` is above `BASE_DEVICE_NROOTS` anyway.
...
| gate | result |
|---|---|
| `range_omega_parity` vs vendored libcint 6.1.3, ω sweep × 10 tuples, `rys_order` 1..5 | worst \|diff\| **3.4e-14** |
| `range_omega_parity` wide sweep — 4 exponents × 3 separations × 4 ω, `rys_order` 4/4/5/7, `lower` past 0.999 | worst scaled \|diff\| **< 1e-8** |
| `SR(ω) + LR(ω) == full`, every tuple | ≤ 1e-12 rel (doubled roots), ≤ 1e-10 (`sr_rys_roots`) |
| `sr_rys_roots_parity` — kernel-shaped functionals across every `lower`/`x` threshold, `nroots` 1..12 | **≤ 1e-9** (`nroots ≤ 10`), ≤ 1e-7 (11–12) |
| `range_omega = None` | whole suite unchanged |

### What stage 5 inherits

Everything §4.2 named. `_RSGDFBuilder` (14-07 7b/7c), `_RSMDFBuilder`, `RSDF`,
**Gate 3**, 14-07 Task 7d's `_prefer_ccdf` flip, `rsjk` — and, because stage 3
landed alongside stage 2, `rsjk` and RSH on d/f bases as well, which §4.3 had
deferred.

Two things to know before starting it:

* **Thread ω through `SessionRequest` options, before `query_workspace`.** It
  sizes the workspace; changing it afterwards is rejected as contract drift. No
  change to `build_image_expanded_with_aux` — the ω travels in the options, not
  in the basis, which is why §1c was never an obstacle to the real fix.
* **`aux_e2` reaches `int3c2e` and `int2c2e`, and `rsjk` reaches `int2e`. All
  three are supported. Derivative rows are not** — an `int3c2e_ip1` under a set
  ω returns `UnsupportedApi` naming `range_omega`. If a gradient path needs it,
  that is a follow-up, not a surprise at runtime.
