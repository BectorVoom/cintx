...
- [x] **Phase 18: SessionRequest Arity ≥3 Dispatch (issue #11 Task 2)** (completed 2026-05-12) - Extend `SessionRequest::evaluate` to dispatch arity-3 and arity-4 shell tuples (covering `int2e_*`, `int3c1e*`, `int3c2e_*`, `int4c1e_*`) through the existing operator catalog with F-order AO layout matching libcint memory layout.
...
| Phase 18: SessionRequest Arity ≥3 Dispatch | v1.3 | 4/4 | Complete | 2026-05-12 |
...
**Success Criteria** (what must be TRUE):
  ...
  4. `manifest-audit --check-lock` is green under `CINTX_PARITY_STRICT=1`, i.e. `unsupported_libcint_families` is EMPTY and the `PARITY_BASELINE` (introduced by gradient-gap Wave 5, `xtask/src/manifest_audit.rs`) has been emptied of its Phase-30/31 entries. The reference set is the union of `cint_funcs.h`, the supplemental headers, AND every `ALL_CINT(...)`/`ALL_CINT1E(...)` export in `libcint-master/src/**/*.c` — the widened set Wave 0 added, because 33 derivative symbols are exported by the built library while absent from `cint_funcs.h`. Every libcint 6.1.3 family is `oracle_covered=true` for its physical representations (cart/sph; spinor where physical, σ families spinor-only), and the full vendor-gated oracle suite is green under both flags (PARITY-01).

     **Two documented exceptions, both established by Wave 5 and neither closable by any amount of work:** rows carrying `unsupported_policy = "no_upstream_oracle"` (six spinor rows whose vendored libcint driver is an unconditional stub) and `int1e_ecp_iprinv_spinor` (no ECP oracle exists in libcint or PySCF). These stay `oracle_covered=false`; the criterion is met when no OTHER row does. See `.planning/notes/gradient-gap-wave-5-PLAN.md` §0.1–0.2.
