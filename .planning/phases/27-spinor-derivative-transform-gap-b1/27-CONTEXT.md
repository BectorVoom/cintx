...
- **D-02 (REVISED 2026-05-31):** Also flip the **arity-3** family `int3c2e_ip1/ip2_spinor` — a derivative wrapper over `cart_to_spinor_sf_3c2e`. Its scalar cart forms are confirmed `oracle_covered=true` AND it has a real libcint 6.1.3 vendor byte-identity reference. **`int3c1e_ip1/iprinv_spinor` and `int2c2e_ip1/ip2_spinor` are REMOVED from this flip scope — see D-12.**
...
- **D-12 (DEFERRED — vendor-stub gap, decided 2026-05-31 at the Wave-3 pause checkpoint):** `int2c2e_ip1/ip2_spinor` and `int3c1e_ip1/iprinv_spinor` (4 arms) stay `oracle_covered=false`. **Ground truth:** libcint 6.1.3 ships *stubs* for exactly these spinor-derivative drivers — `int2c2e_ip1/ip2_spinor` return 0 and `CINT3c1e_spinor_drv` calls `exit(1)` — so **no vendor byte-identity reference is achievable** and the original D-02 flip for them is disproven. cintx's scalar cart forms for these are covered, but byte-identity verification cannot be performed against an upstream that does not implement them. Honest-scope decision (user, pause checkpoint): keep these 4 arms `false` this phase, flip only the vendor-backed families. Finite-difference (numerical-gradient) verification against the covered scalar families is a **deferred follow-up** — see Deferred Ideas. This is NOT a cintx defect; it is an upstream-vendor limitation.
...
- **Finite-difference verification for the 4 vendor-stub arms** (`int2c2e_ip1/ip2_spinor`, `int3c1e_ip1/iprinv_spinor`, per D-12) — libcint 6.1.3 has no working driver for these, so byte-identity is impossible. A follow-up could verify cintx's output via central finite-difference of the covered scalar `int2c2e_spinor` / `int3c1e_spinor` families (numerical gradient ≈ analytic ip kernel), then flip `oracle_covered=true` under an FD-tolerance gate distinct from the vendor byte-identity gate. Out of scope for this phase by user decision at the Wave-3 pause.
