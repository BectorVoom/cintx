...
must_haves:
  truths:
    ...
    - "Cart, spherical, and spinor outputs route through explicit transform code before caller-visible writes, preserving upstream-compatible shapes and interleaved spinor layout."
  ...
  key_links:
    ...
    - from: crates/cintx-cubecl/src/executor.rs
      ...
      via: "Kernel outputs route through the representation-specific transform helpers before host-visible writes."
...
<verification>
Run the CubeCL library tests after both tasks to prove the family registry now covers the full Phase 2 base set and the representation-specific transform paths are active.
</verification>

<success_criteria>
The CubeCL backend can execute the remaining `3c1e` and `3c2e` families and route results through the correct cart/sph/spinor transform paths without expanding Phase 2 to unsupported families.
</success_criteria>
