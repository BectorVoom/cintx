...
- [x] Remaining Group-1 first-derivative families (DRV1-01..05): `int2e_ip2`, `int1e_ipovlpip/ipkinip/ipnucip` (rank-9 both-side), `int3c1e_ip1`/`int3c1e_iprinv`, `int2c2e_ip1`/`int2c2e_ip2`, and `int3c2e_ip2` all reach byte-identity vs vendored libcint 6.1.3 at atol=1e-12 (cart + sph, component_rank 3) under the `--features cpu` + `CINTX_ORACLE_BUILD_VENDOR=1` double gate. Built by promoting/extending the Phase-21 `f12.rs` single-side engine: `nabla1j/k/l_2e` + a `Nabla1Center{I,J,K,L}`-parameterized `gout_ipn`; `int3c2e_ip2` differentiates the auxiliary k via the 2e `ll` slot (`nabla1l_2e`); `int3c1e_iprinv` adds the only new base (`fill_g_tensor_3c1e_nuc`, Rys-driven, reusing `rys_roots_host` + `PTR_RINV_ORIG`). Spinor reps registered → `UnsupportedApi`; no capi/legacy surface added; manifest operators appended (OperatorId == lock position). Known follow-up: int3c1e gradient launchers contract only nctr column 0 (correct for nctr==1; latent for nctr>1, tracked todo). Validated in Phase 23: Group 1 — Remaining 1st-Derivative Families (v1.4).
...
---
*Last updated: 2026-05-30 — Phase 23 complete (DRV1-01..05 remaining Group-1 1st-derivative families byte-identical to libcint 6.1.3, cart+sph); v1.4 in progress*
