...
fn main() {
    ...
    build
        ...
        // Phase 29 REL-04 (BLOCKING): the relativistic 2e σ Gaunt/DKB drivers.
        // gaunt1.c provides int2e_ssp1ssp2/ssp1sps2/sps1ssp2/sps1sps2_spinor;
        // dkb.c provides int2e_spv1/vsp1/spv1spv2/vsp1spv2/spv1vsp2/vsp1vsp2/
        // spv1spsp2/vsp1spsp2_spinor. NEITHER was in the vendor build before
        // (RESEARCH Pitfall 1) — without them REL-04 has no vendor reference
        // to link (undefined-symbol link error). Both use the same include set
        // and cc flags as intor4.c (no suppl-header; all symbols in cint_funcs.h).
        .file(libcint_root.join("src/autocode/gaunt1.c"))
        .file(libcint_root.join("src/autocode/dkb.c"))
        ...
        // Gradient-gap Tier 6 (LRESC): int1e_iprinvr, int1e_iprinviprip,
        // and int1e_ipiprinvrip are compiled exports from lresc.c.
        .file(libcint_root.join("src/autocode/lresc.c"))
        // Phase 14 unstable-source family source files.
...
/* D-PBC-24 stage 3: the LOWER-BOUNDED Rys quadrature (rys_roots.h:4). This is
   the byte-identity reference for `cintx_cubecl::math::rys_wheeler::
   sr_rys_roots_host`, the short-range arm for rys_order > 3. */
extern int CINTsr_rys_roots(int nroots, double x, double lower, double *u, double *w);
