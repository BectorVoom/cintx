//! Randomized ROCm vendor-parity oracle for `int1e_{r2,r4}_origi_sph`
//! (quick task 260529-twi, Task 2).
//!
//! Exercises the CubeCL `origi_scalar_kernel` GPU device path (the scalar origi
//! r2/r4 origin-displaced 1e operators, ported to a `#[cube(launch)]` device
//! kernel and dispatched on the ROCm `HipRuntime` in quick-260529-twi) under
//! `CINTX_BACKEND=rocm` over many RANDOMIZED H2O/STO-3G systems.
//!
//! For each random system and each of the two scalar origi operators, a varied
//! shell-pair table is evaluated on the ROCm device via the RAW API
//! (`eval_raw` with `RawApiId::Symbol("int1e_r2_origi_sph")` /
//! `("int1e_r4_origi_sph")` — the unstable-source families are driven through
//! the raw API by Symbol, NOT a raw-API enum variant) and compared ELEMENT-WISE
//! against the vendored libcint 6.1.3 oracle
//! (`vendor_ffi::vendor_int1e_r2_origi_sph` / `_r4_origi_sph`). The suite asserts
//! `mismatch_count == 0` at atol=1e-12 / rtol=0 AND `any_nonzero == true`
...
//!   --test origi_random_rocm_parity -- --ignored
...
/// Evaluate one origi scalar shell-pair block via `eval_raw` on the resolved
/// (ROCm) backend (CINTX_BACKEND=rocm).
fn eval_origi_block(
    symbol: &'static str,
    shls: &[i32; 2],
    atm: &[i32],
...
/// Randomized `int1e_{r2,r4}_origi_sph` ROCm vendor-parity oracle.
#[test]
#[ignore]
fn test_int1e_origi_scalar_random_rocm_parity() {
    assert_eq!(
    ...
    let mut rng = Lcg::new(0x0719_2605_29c0_de17_u64);

    let operators: [&str; 2] = ["int1e_r2_origi_sph", "int1e_r4_origi_sph"];
    ...
    for sys in 0..n_systems {
        ...
        for &symbol in &operators {
            for shls in shell_pairs {
                case_count += 1;
                let cintx_out = eval_origi_block(symbol, &shls, &atm, &bas, &env);
                ...
                #[cfg(has_vendor_libcint)]
                {
                    ...
                    for (idx, (&r, &o)) in vendor_out.iter().zip(cintx_out.iter()).enumerate() {
                        ...
                        if diff > atol {
                            mismatch_count += 1;
                            eprintln!(
                                "  rocm origi RANDOM MISMATCH sys {sys} op {symbol} shls {shls:?} \
    ...
    assert_eq!(
        mismatch_count, 0,
        "rocm origi oracle parity failed: {mismatch_count} mismatches across \
         {case_count} random int1e_{{r2,r4}}_origi_sph cases vs vendor at atol={atol:.0e}"
    );
    assert!(
        any_nonzero,
        "origi device output was all zeros across {case_count} cases — \
         the device kernel did not run on the GPU"
    );
    println!(
        "  PASS: rocm int1e_{{r2,r4}}_origi_sph random vendor-parity mismatch_count=0 \
