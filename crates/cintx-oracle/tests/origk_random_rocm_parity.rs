//! Randomized ROCm vendor-parity oracle for the SCALAR origk 3c1e device path
//! (quick task 260529-twi Task 5).
//!
//! Drives the `#[cube(launch)] origk_scalar_kernel` device kernel on the ROCm
//! `HipRuntime` via the RAW API (`eval_raw` with
//! `RawApiId::Symbol("int3c1e_{r2,r4,r6}_origk_sph")`) over randomized H2O/STO-3G
//! 3-shell triples and compares the output element-for-element vs the vendored
//! libcint 6.1.3 oracle (`vendor_int3c1e_{r2,r4,r6}_origk_sph`) — proving both
//! "the device kernel ran on the GPU" AND "0 divergence vs libcint".
//!
...
//!     --test origk_random_rocm_parity -- --ignored
//!
...
///
/// Env layout (PTR_ENV_START=20-aligned fixture): O coord @20, H1 @23, H2 @26,
/// zeta @29, O-1s exp @30, O-2s exp @36, O-2p exp @42, H-1s exp @48.
...
/// Randomized origk `int3c1e_{r2,r4,r6}_origk_sph` ROCm vendor-parity oracle for
/// the scalar device path. 144 cases (8 random geometries × 6 triples × 3 r_powers).
#[test]
#[ignore]
fn test_int3c1e_origk_scalar_random_rocm_parity() {
    assert_eq!(
    ...
    let mut rng = Lcg::new(0x2605_2917_0716_c0de_u64);
    ...
    for _geo in 0..n_geometries {
        ...
        for &[i_sh, j_sh, k_sh] in SHELL_TRIPLES.iter() {
            ...
            // origk emits spherical i/j AND spherical k (k_cartesian=false).
            let n_elem = nsph(li) * nsph(lj) * nsph(lk);
    ...
    println!(
        "origk_random_rocm_parity: cases={cases} mismatch_count={mismatch_count} any_nonzero={any_nonzero}"
    );
    assert_eq!(
        mismatch_count, 0,
        "scalar origk ROCm device output diverged from vendor in {mismatch_count} elements across {cases} cases"
    );
    assert!(
        any_nonzero,
        "all origk outputs were zero — the device kernel did not actually run (all-zeros fallback)"
    );
}
