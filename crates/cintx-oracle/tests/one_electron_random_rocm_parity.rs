//! Randomized ROCm idempotency oracle for `int1e_{ovlp,kin,nuc}_sph`
//! (quick task 260529-imi).
//!
//! Exercises the CubeCL `one_electron_scalar_kernel` GPU device path (the scalar
//! 1e overlap / kinetic / nuclear-attraction arms, ported to a `#[cube(launch)]`
//! device kernel and dispatched on the ROCm `HipRuntime` in quick-260529-imi)
//! under `CINTX_BACKEND=rocm` over many RANDOMIZED H2O/STO-3G systems.
//!
//! For each random system and each of the three scalar operators, the full
//! shell-pair AO matrix is evaluated TWICE on the ROCm device via the RAW API
//! (`eval_raw` with `RawApiId::{INT1E_OVLP_SPH, INT1E_KIN_SPH, INT1E_NUC_SPH}` —
//! the 1e family has raw-API enum variants, so it is driven through the raw API
//! like the f12 oracle, NOT the safe API like the ECP oracle). The two runs must
//! agree exactly (idempotency); the suite as a whole must produce non-zero output
...
//! feature gate — single gate like the ECP oracle, NOT the f12 double gate) +
//! `#[test] #[ignore]` + `CINTX_ROCM_ORACLE=1` runtime gate. It is then picked up
//! by `xtask rocm-oracle`'s BASE-profile
...
// ecp_random_rocm_parity.rs / f12_random_rocm_parity.rs).
// ─────────────────────────────────────────────────────────────────────────────
...
// one_electron_parity.rs::build_h2o_sto3g — that builder is test-local, not
// exported from cintx_oracle::fixtures). The env exponent offsets are returned
...
/// Evaluate the full `int1e_*_sph` AO matrix once via `eval_raw` on the resolved
/// (ROCm) backend. The backend is resolved from `CINTX_BACKEND=rocm` exported by
/// `xtask rocm-oracle` (executor.rs resolve_backend_kind), NOT from options.
/// Mirrors `one_electron_parity.rs::collect_1e_sph_matrix`.
fn collect_1e_matrix_once(api_id: RawApiId, atm: &[i32], bas: &[i32], env: &[f64]) -> Vec<f64> {
...
/// Randomized `int1e_{ovlp,kin,nuc}_sph` ROCm idempotency oracle.
///
/// Drives the raw-API scalar 1e path TWICE per random H2O/STO-3G system on the
...
/// `one_electron_scalar_kernel` `#[cube(launch)]` device kernel ran on the AMD GPU).
#[test]
#[ignore]
fn test_int1e_scalar_random_rocm_idempotency() {
    assert_eq!(
    ...
    let rtol = 1e-10_f64;
    let n_cases = 48usize;
    let mut rng = Lcg::new(0x5ec0_ec90_2605_291a_u64);

    let operators: [(&str, RawApiId); 3] = [
        ("int1e_ovlp_sph", RawApiId::INT1E_OVLP_SPH),
        ("int1e_kin_sph", RawApiId::INT1E_KIN_SPH),
        ("int1e_nuc_sph", RawApiId::INT1E_NUC_SPH),
    ];
    ...
    for case in 0..n_cases {
        ...
        for (op_name, api_id) in operators {
            let out1 = collect_1e_matrix_once(api_id, &atm, &bas, &env);
            let out2 = collect_1e_matrix_once(api_id, &atm, &bas, &env);
            ...
            for (idx, (&r, &o)) in out1.iter().zip(out2.iter()).enumerate() {
                ...
                if diff > threshold {
                    mismatch_count += 1;
                    eprintln!(
                        "  rocm 1e RANDOM MISMATCH case {case} op {op_name} idx {idx}: \
    ...
    assert_eq!(
        mismatch_count, 0,
        "rocm 1e oracle idempotency failed: {mismatch_count} mismatches across \
         {n_cases} random int1e_{{ovlp,kin,nuc}}_sph cases"
    );
    assert!(
        any_nonzero,
        "1e device output was all zeros across {n_cases} cases — \
         the device kernel did not run on the GPU"
    );
    println!(
        "  PASS: rocm int1e_{{ovlp,kin,nuc}}_sph random idempotency mismatch_count=0 \
