use crate::contracts::{
    BasisSet, IntegralFamily, Operator, OperatorKind, Representation, validate_dims,
};
...
pub fn plan_execution(
    ...
    let mut shell_angular_momentum = Vec::with_capacity(request.shell_tuple.len());
    ...
    enforce_optional_family_policy(
        Operator::new(request.operator.family, request.operator.kind).map_err(|_| {
        ...
        &natural_dims,
        &dims,
        &request.memory.backend_candidate,
        request
            .memory
            .feature_flags
            .iter()
...
fn family_arity(family: IntegralFamily) -> usize {
    match family {
        ...
        IntegralFamily::TwoElectron | IntegralFamily::FourCenterOneElectron => 4,
    }
}
