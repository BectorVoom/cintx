...
use crate::runtime::policy::{WITH_4C1E_FEATURE, feature_flag_enabled};
...
impl CpuRouteKey {
    ...
    pub const fn canonical_family(self) -> &'static str {
        match self.family {
            ...
            IntegralFamily::FourCenterOneElectron => "4c1e",
        }
    }
...
const fn optional_implemented_entry(
    route_id: &'static str,
    key: CpuRouteKey,
    ...
    CpuRouteManifestEntry {
        ...
        surface_group: RouteSurfaceGroup::All,
        feature_flag,
        stability: RouteStability::Optional,
        support_predicate,
        route_kind,
        backend_set: BACKEND_CPU,
        entry_kernel,
...
const fn optional_unsupported_entry(
    route_id: &'static str,
    key: CpuRouteKey,
    feature_flag: &'static str,
    support_predicate: &'static str,
...
static ROUTE_COVERAGE_MANIFEST: [CpuRouteManifestEntry; 26] = [
    implemented_entry(
    ...
    implemented_entry(
        ...
        RouteOptimizerMode::Supported,
        "tests/two_c_wrapper_parity.rs",
        NO_TRANSFORMS,
        "wrapper-backed two-center two-electron Cartesian parity route",
    ),
    implemented_entry(
        ...
        "tests/two_c_wrapper_parity.rs",
        NO_TRANSFORMS,
        "wrapper-backed two-center two-electron spherical parity route",
    ),
    implemented_entry(
        ...
        "tests/two_c_wrapper_parity.rs",
        NO_TRANSFORMS,
        "wrapper-backed two-center two-electron spinor parity route",
    ),
    implemented_entry(
    ...
    optional_implemented_entry(
        ...
        "libcint_flat_col_major_4c1e",
        RouteOptimizerMode::IgnoredButInvariant,
        "tests/four_c_wrapper_parity.rs",
        NO_TRANSFORMS,
        "four-center one-electron Cartesian route gated by Validated4C1E policy; optimizer/cache pointers are forwarded but optimization is treated as invariant-only for this validated envelope",
    ),
    optional_implemented_entry(
        ...
        "tests/four_c_wrapper_parity.rs",
        NO_TRANSFORMS,
        "four-center one-electron spherical route gated by Validated4C1E policy; optimizer/cache pointers are forwarded but optimization is treated as invariant-only for this validated envelope",
    ),
    optional_unsupported_entry(
        ...
        WITH_4C1E_FEATURE,
        "unsupported_policy",
        "4c1e spinor is outside Validated4C1E because upstream int4c1e_spinor is not implemented",
    ),
    unsupported_entry(
...
pub fn resolve_route_request(
    ...
    resolve_route_with_request(request, surface)
}
...
pub fn resolve_route(
    ...
    for entry in ROUTE_COVERAGE_MANIFEST {
        ...
        if entry.status == RouteStatus::Implemented
            && entry_selectable_without_request(entry.feature_flag, entry.support_predicate)
        {
...
fn resolve_route_with_request(
    request: &ExecutionRequest,
    surface: RouteSurface,
) -> Result<ResolvedCpuRoute, LibcintRsError> {
    let key = CpuRouteKey::from(request);
    let mut implemented: Option<CpuRouteManifestEntry> = None;
    let mut matched_key_surface = false;
    ...
    for entry in ROUTE_COVERAGE_MANIFEST {
        ...
        matched_key_surface = true;
        if !entry_feature_enabled(entry.feature_flag, request) {
            continue;
        }
        if !entry_support_predicate_matches(entry.support_predicate, request) {
            continue;
        }
        if entry.status == RouteStatus::Implemented {
            if let Some(previous) = implemented {
...
fn entry_feature_enabled(entry_feature_flag: &str, request: &ExecutionRequest) -> bool {
    if entry_feature_flag == "none" {
        return true;
    }

    feature_flag_enabled(
        request.memory.feature_flags.iter().map(String::as_str),
        entry_feature_flag,
    )
}

fn entry_selectable_without_request(entry_feature_flag: &str, support_predicate: &str) -> bool {
    entry_feature_flag == "none" && support_predicate == "always"
}

fn entry_support_predicate_matches(support_predicate: &str, request: &ExecutionRequest) -> bool {
    match support_predicate {
        "always" => true,
        "Validated4C1E" => {
            request.memory.backend_candidate == "cpu"
                && matches!(
                    request.representation,
