...
pub(crate) fn raw_route_cache_required_len(
    ...
    match route.entry_kernel {
        ...
        | RouteEntryKernel::Direct(CpuKernelSymbol::Int2c2eCart)
        | RouteEntryKernel::Direct(CpuKernelSymbol::Int2c2eSph)
        | RouteEntryKernel::Direct(CpuKernelSymbol::Int2c2eSpinor) => {
            let symbol = match route.entry_kernel {
                RouteEntryKernel::Direct(symbol) => symbol,
                _ => unreachable!("matched only direct two-electron kernels"),
            };
            query_two_e_cache_required_len(symbol, dims, shell_tuple, atm, bas, env, opt_ptr)
        }
        ...
        | RouteEntryKernel::ThreeCenterOneElectronSpinorAdapter => {
            let symbol = match route.entry_kernel {
                ...
                | RouteEntryKernel::Direct(CpuKernelSymbol::Int3c2eIp1Cart)
                | RouteEntryKernel::Direct(CpuKernelSymbol::Int3c2eIp1Sph) => {
                    three_center_real_symbol(route.entry_kernel)?
                }
                RouteEntryKernel::Direct(CpuKernelSymbol::Int3c2eIp1Spinor)
                | RouteEntryKernel::ThreeCenterOneElectronSpinorAdapter => {
                    three_center_spinor_symbol(route.entry_kernel)?
                }
                _ => unreachable!("matched only three-center kernels"),
            };
            query_three_center_cache_required_len(symbol, dims, shell_tuple, atm, bas, env, opt_ptr)
        }
        RouteEntryKernel::Direct(CpuKernelSymbol::Int4c1eCart)
        | RouteEntryKernel::Direct(CpuKernelSymbol::Int4c1eSph) => {
            let symbol = four_center_real_symbol(route.entry_kernel)?;
            query_four_center_cache_required_len(symbol, dims, shell_tuple, atm, bas, env, opt_ptr)
        }
        _ => Ok(0),
    }
}

fn raw_opt_ptr(opt: Option<NonNull<c_void>>) -> *mut c_void {
    opt.map_or(ptr::null_mut(), NonNull::as_ptr)
}

fn raw_cache_ptr(cache: Option<&mut [f64]>) -> *mut f64 {
    cache.map_or(ptr::null_mut(), |values| values.as_mut_ptr())
}
...
pub(crate) fn execute_safe_specialized_route(
    ...
    match route.entry_kernel {
        ...
        RouteEntryKernel::Direct(CpuKernelSymbol::Int4c1eCart)
        | RouteEntryKernel::Direct(CpuKernelSymbol::Int4c1eSph) => {
            execute_safe_four_center_real_route(
                route.entry_kernel,
                basis,
                shell_tuple,
                dims,
...
pub(crate) fn execute_safe_specialized_spinor_route(
    ...
    match route.entry_kernel {
        ...
        | RouteEntryKernel::Direct(CpuKernelSymbol::Int2c2eSpinor) => {
            let symbol = match route.entry_kernel {
                RouteEntryKernel::Direct(symbol) => symbol,
                _ => unreachable!("matched only direct two-electron spinor routes"),
            };
            execute_safe_two_e_spinor_route(symbol, basis, shell_tuple, dims, output)?;
            Ok(true)
        }
        RouteEntryKernel::Direct(CpuKernelSymbol::Int3c2eIp1Spinor)
        | RouteEntryKernel::ThreeCenterOneElectronSpinorAdapter => {
...
pub(crate) fn execute_raw_specialized_route(
    ...
    let opt_ptr = raw_opt_ptr(opt);
    let cache_ptr = raw_cache_ptr(cache);
    match route.entry_kernel {
        RouteEntryKernel::OneElectronOverlapCartesian => {
...
fn execute_raw_two_e_route(
    ...
    match representation {
        Representation::Cartesian | Representation::Spherical => {
            run_two_e_real_kernel(symbol, dims, shell_tuple, atm, bas, env, opt, cache, output)
        }
        Representation::Spinor => run_two_e_spinor_scalars_kernel(
            symbol,
            dims,
            shell_tuple,
            atm,
...
fn execute_safe_four_center_real_route(
    entry_kernel: RouteEntryKernel,
    basis: &BasisSet,
    shell_tuple: &[usize],
    dims: &[usize],
    ...
    let symbol = four_center_real_symbol(entry_kernel)?;
    let raw = safe_basis_to_raw_layout(basis)?;
    run_four_center_real_kernel(
        symbol,
        dims,
        shell_tuple,
        raw.atm.as_slice(),
...
fn execute_raw_four_center_route(
    entry_kernel: RouteEntryKernel,
    atm: &[i32],
    bas: &[i32],
    env: &[f64],
    ...
    let symbol = four_center_real_symbol(entry_kernel)?;
    run_four_center_real_kernel(symbol, dims, shell_tuple, atm, bas, env, opt, cache, output)
}
...
fn execute_raw_three_center_route(
    ...
    let representation = Representation::from(symbol);
    match representation {
        Representation::Cartesian | Representation::Spherical => run_three_center_real_kernel(
            symbol,
            dims,
            shell_tuple,
            atm,
        ...
        Representation::Spinor => run_three_center_spinor_scalars_kernel(
            symbol,
            dims,
            shell_tuple,
            atm,
...
fn four_center_real_symbol(
    entry_kernel: RouteEntryKernel,
) -> Result<CpuKernelSymbol, LibcintRsError> {
    match entry_kernel {
        RouteEntryKernel::Direct(CpuKernelSymbol::Int4c1eCart) => Ok(CpuKernelSymbol::Int4c1eCart),
        RouteEntryKernel::Direct(CpuKernelSymbol::Int4c1eSph) => Ok(CpuKernelSymbol::Int4c1eSph),
        _ => Err(LibcintRsError::BackendFailure {
            backend: "cpu.four_center",
            detail: format!(
                "four-center real execution requires a direct 4c1e real kernel, got `{}`",
                entry_kernel.as_str()
            ),
        }),
    }
...
fn query_two_e_cache_required_len(
    symbol: CpuKernelSymbol,
    dims: &[usize],
    shell_tuple: &[usize],
    atm: &[i32],
    ...
    let result = unsafe {
        match Representation::from(symbol) {
            Representation::Cartesian | Representation::Spherical => ffi::call_two_e_real_kernel(
                symbol,
                ptr::null_mut::<f64>(),
                dims_i32.as_mut_ptr(),
                shls_i32.as_mut_ptr(),
            ...
            Representation::Spinor => ffi::call_two_e_spinor_kernel(
                symbol,
                ptr::null_mut::<c_void>(),
                dims_i32.as_mut_ptr(),
                shls_i32.as_mut_ptr(),
    ...
    normalize_cache_len_result("cpu.two_e", symbol, result)
}

#[allow(clippy::too_many_arguments)]
fn query_three_center_cache_required_len(
    symbol: CpuKernelSymbol,
    dims: &[usize],
    shell_tuple: &[usize],
    atm: &[i32],
    ...
    let result = unsafe {
        match Representation::from(symbol) {
            ...
            Representation::Spinor => ffi::call_three_center_spinor_kernel(
                symbol,
                ptr::null_mut::<c_void>(),
                dims_ptr,
                shls_i32.as_mut_ptr(),
    ...
    normalize_cache_len_result("cpu.three_center", symbol, result)
}

#[allow(clippy::too_many_arguments)]
fn query_four_center_cache_required_len(
    symbol: CpuKernelSymbol,
    dims: &[usize],
    shell_tuple: &[usize],
    atm: &[i32],
    ...
    let result = unsafe {
        ffi::call_four_center_real_kernel(
            symbol,
            ptr::null_mut::<f64>(),
            dims_i32.as_mut_ptr(),
            shls_i32.as_mut_ptr(),
    ...
    normalize_cache_len_result("cpu.four_center", symbol, result)
}

fn normalize_cache_len_result(
    backend: &'static str,
    symbol: CpuKernelSymbol,
    result: Option<i32>,
) -> Result<usize, LibcintRsError> {
    let cache_len = result.ok_or_else(|| LibcintRsError::BackendFailure {
        backend,
        detail: format!(
            "route selected unsupported kernel symbol `{}` during cache-size query",
            symbol.name()
        ),
    })?;
    if cache_len < 0 {
        return Err(LibcintRsError::BackendFailure {
            backend,
            detail: format!(
                "kernel `{}` returned invalid cache-size status {cache_len}",
                symbol.name()
            ),
        });
    }

    usize::try_from(cache_len).map_err(|_| LibcintRsError::BackendFailure {
        backend,
        detail: format!(
            "kernel `{}` cache-size result does not fit usize: {cache_len}",
            symbol.name()
        ),
    })
}
...
#[allow(clippy::too_many_arguments)]
fn run_four_center_real_kernel(
    symbol: CpuKernelSymbol,
    dims: &[usize],
    shell_tuple: &[usize],
    atm: &[i32],
    ...
    let not0 = result.ok_or_else(|| LibcintRsError::BackendFailure {
        backend: "cpu.four_center",
        detail: format!(
            "route selected non-four-center kernel symbol `{}` for real execution",
            symbol.name()
        ),
    })?;
