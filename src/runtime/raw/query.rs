...
use crate::runtime::backend::cpu::{raw_route_cache_required_len, resolve_raw_route};
use crate::runtime::execution_plan::ExecutionRequest;
use crate::runtime::executor::build_memory_policy_outcome;
use crate::runtime::policy::enforce_optional_family_policy;
...
pub fn query_workspace_compat(
    ...
    enforce_optional_family_policy(
        operator,
        representation,
        &validated.shell_angular_momentum,
        &validated.natural_dims,
        &validated.dims,
        options.backend_candidate,
        options.feature_flags.iter().copied(),
    )
    ...
    let resolved_route = resolve_raw_route(&execution_request)
        .map_err(|error| diagnostics.clone().record_failure("routing", error))?;
    let route_cache_required_len = raw_route_cache_required_len(
        resolved_route,
        request.atm,
        request.bas,
        request.env,
        validated.shell_tuple.as_slice(),
        validated.dims.as_slice(),
        request.opt,
    )
    .map_err(|error| diagnostics.clone().record_failure("routing", error))?;
    if cache_provided_len > 0 && cache_provided_len < route_cache_required_len {
        return Err(diagnostics.clone().record_failure(
            "validation",
            LibcintRsError::InvalidLayout {
                item: "cache_length",
                expected: route_cache_required_len,
                got: cache_provided_len,
            },
        ));
    }
    ...
    Ok(RawCompatWorkspace {
        ...
        cache_required_len: route_cache_required_len,
