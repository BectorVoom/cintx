...
use cintx::{
    CpuRouteKey, CpuRouteTarget, IntegralFamily, LibcintRsError, ManifestProfile, OperatorKind,
...
fn stable_wrapper_oracle_scalars(
    ...
    let stable_two_c2e_key = CpuRouteKey::new(
    ...
    if route_key == stable_two_c2e_key {
        let expected_dims: &[usize] = match representation {
            Representation::Cartesian | Representation::Spherical => &[1, 3],
            Representation::Spinor => &[2, 6],
        };
        if dims != expected_dims {
            return Err(LibcintRsError::InvalidInput {
                field: "dims",
                reason: format!(
                    "wrapper-backed stable 2c2e oracle only supports dims {expected_dims:?} for {representation:?}, got {dims:?}"
                ),
            });
        }
        ...
        return match representation {
            Representation::Cartesian | Representation::Spherical => {
                let (out, shape) = cint
                    .integrate("int2c2e", None, [[0usize, 1usize], [1usize, 2usize]])
                    .into();
                assert_eq!(shape, dims);
                Ok(Some(out))
            }
            Representation::Spinor => {
                let (out, shape) = cint
                    .integrate_spinor("int2c2e", None, [[0usize, 1usize], [1usize, 2usize]])
                    .into();
                assert_eq!(shape, dims);
                let mut flattened = Vec::with_capacity(out.len() * 2);
                for value in out {
