...
fn spherical_safe_evaluate_matches_wrapper_for_reduced_one_e_case() {
    ...
    let options = parity_options(&["one-e-nuclear-spherical-safe-wrapper"]);
    ...
    .expect("safe spherical nuclear evaluate must succeed");
    let safe_scalars = flatten_real_output(safe_tensor.output);

    let (wrapper_scalars, wrapper_dims) =
        wrapper_nuclear_spherical(&atm, &bas, &env, REDUCED_D_P_RAW_SHLS);
    ...
    assert_within_tolerance(
        &wrapper_scalars,
        &safe_scalars,
        "safe spherical nuclear vs wrapper reduced d/p case",
    );
}
...
fn spherical_safe_evaluate_into_matches_wrapper_for_reduced_one_e_case() {
    let basis = stable_safe_basis();
    let (atm, bas, env) = stable_raw_layout();
    let options = parity_options(&["one-e-nuclear-spherical-safe-evaluate-into-wrapper"]);

    let (_, wrapper_dims) = wrapper_nuclear_spherical(&atm, &bas, &env, REDUCED_D_P_RAW_SHLS);
    ...
    .expect("safe spherical nuclear evaluate_into must succeed");
    ...
    assert_within_tolerance(
        &wrapper_scalars,
        &safe_output,
        "safe spherical nuclear evaluate_into vs wrapper reduced d/p case",
    );
}
...
fn spherical_raw_matches_wrapper_for_reduced_one_e_case() {
    let (atm, bas, env) = stable_raw_layout();
    let options = parity_options(&["one-e-nuclear-spherical-raw-wrapper"]);

    let (workspace, raw_scalars) =
        evaluate_raw_spherical_nuclear(&atm, &bas, &env, REDUCED_D_P_RAW_SHLS, &options);
    let (wrapper_scalars, wrapper_dims) =
        wrapper_nuclear_spherical(&atm, &bas, &env, REDUCED_D_P_RAW_SHLS);
    ...
    assert_within_tolerance(
        &wrapper_scalars,
        &raw_scalars,
        "raw spherical nuclear vs wrapper reduced d/p case",
    );
}
...
fn spherical_raw_uses_libcint_flat_layout() {
    let (atm, bas, env) = stable_raw_layout();
    let options = parity_options(&["one-e-nuclear-spherical-layout"]);
    ...
    assert_within_tolerance(
        &wrapper_scalars,
        &raw_scalars,
        "raw spherical nuclear must preserve libcint flat column-major layout",
    );

    let row_major_max_diff = max_abs_diff(&row_major_scalars, &raw_scalars);
    assert!(
        row_major_max_diff > 1e-9,
        "raw spherical nuclear unexpectedly aligns with row-major flattening"
    );
}
...
fn spherical_raw_interprets_atm_bas_env_and_shls_like_wrapper() {
    let (atm, bas, env) = stable_raw_layout();
    let options = parity_options(&["one-e-nuclear-spherical-raw-inputs"]);

    for &shls in &[&[0, 1][..], REDUCED_D_P_RAW_SHLS, &[2, 2][..], &[3, 2][..]] {
        let (workspace, raw_scalars) =
            evaluate_raw_spherical_nuclear(&atm, &bas, &env, shls, &options);
        let (wrapper_scalars, wrapper_dims) = wrapper_nuclear_spherical(&atm, &bas, &env, shls);

        assert_eq!(workspace.dims, wrapper_dims, "dims drift for shls={shls:?}");
        assert_within_tolerance(
            &wrapper_scalars,
            &raw_scalars,
            &format!("raw spherical nuclear shell interpretation shls={shls:?}"),
        );
    }
}
...
fn spherical_raw_is_optimizer_invariant_against_wrapper() {
    let (atm, bas, env) = stable_raw_layout();
    let baseline_options = phase3_optimizer_options(&["one-e-nuclear-spherical-optimizer-off"]);
    let optimized_options = phase3_optimizer_options(&["one-e-nuclear-spherical-optimizer-on"]);
    ...
    assert_within_tolerance(
        &baseline_output,
        &optimized_output,
        "raw spherical nuclear optimizer on/off parity",
    );
    assert_within_tolerance(
        &wrapper_scalars,
        &baseline_output,
        "raw spherical nuclear optimizer-off vs wrapper",
    );
    assert_within_tolerance(
        &wrapper_scalars,
        &optimized_output,
        "raw spherical nuclear optimizer-on vs wrapper",
    );
}
...
fn spinor_safe_evaluate_matches_wrapper_for_reduced_one_e_case() {
    let basis = stable_safe_basis();
    let (atm, bas, env) = stable_raw_layout();
    let options = parity_options(&["one-e-nuclear-spinor-safe-wrapper"]);
    ...
    .expect("safe spinor nuclear evaluate must succeed");
    let safe_scalars = flatten_spinor_output(safe_tensor.output);

    let (wrapper_scalars, wrapper_dims) =
        wrapper_nuclear_spinor(&atm, &bas, &env, REDUCED_D_P_RAW_SHLS);
    ...
    assert_within_tolerance(
        &wrapper_scalars,
        &safe_scalars,
        "safe spinor nuclear vs wrapper reduced d/p case",
    );
}
...
fn spinor_safe_evaluate_into_matches_wrapper_for_reduced_one_e_case() {
    let basis = stable_safe_basis();
    let (atm, bas, env) = stable_raw_layout();
    let options = parity_options(&["one-e-nuclear-spinor-safe-evaluate-into-wrapper"]);

    let (_, wrapper_dims) = wrapper_nuclear_spinor(&atm, &bas, &env, REDUCED_D_P_RAW_SHLS);
    let element_count = checked_product(&wrapper_dims);
    ...
    .expect("safe spinor nuclear evaluate_into must succeed");
    ...
    assert_within_tolerance(
        &wrapper_scalars,
        &flattened_safe,
        "safe spinor nuclear evaluate_into vs wrapper reduced d/p case",
    );
}
...
fn spinor_raw_matches_wrapper_for_reduced_one_e_case() {
    let (atm, bas, env) = stable_raw_layout();
    let options = parity_options(&["one-e-nuclear-spinor-raw-wrapper"]);

    let (workspace, raw_scalars) =
        evaluate_raw_spinor_nuclear(&atm, &bas, &env, REDUCED_D_P_RAW_SHLS, &options);
    let (wrapper_scalars, wrapper_dims) =
        wrapper_nuclear_spinor(&atm, &bas, &env, REDUCED_D_P_RAW_SHLS);
    ...
    assert_within_tolerance(
        &wrapper_scalars,
        &raw_scalars,
        "raw spinor nuclear vs wrapper reduced d/p case",
    );
}
...
fn spinor_raw_uses_libcint_flat_layout() {
    let (atm, bas, env) = stable_raw_layout();
    let options = parity_options(&["one-e-nuclear-spinor-layout"]);
    ...
    assert_within_tolerance(
        &wrapper_scalars,
        &raw_scalars,
        "raw spinor nuclear must preserve libcint flat column-major layout",
    );

    let row_major_max_diff = max_abs_diff(&row_major_scalars, &raw_scalars);
    assert!(
        row_major_max_diff > 1e-9,
        "raw spinor nuclear unexpectedly aligns with row-major flattening"
    );
}
...
fn spinor_raw_interprets_atm_bas_env_and_shls_like_wrapper() {
    let (atm, bas, env) = stable_raw_layout();
    let options = parity_options(&["one-e-nuclear-spinor-raw-inputs"]);

    for &shls in &[&[0, 1][..], REDUCED_D_P_RAW_SHLS, &[2, 2][..], &[3, 2][..]] {
        let (workspace, raw_scalars) =
            evaluate_raw_spinor_nuclear(&atm, &bas, &env, shls, &options);
        let (wrapper_scalars, wrapper_dims) = wrapper_nuclear_spinor(&atm, &bas, &env, shls);

        assert_eq!(workspace.dims, wrapper_dims, "dims drift for shls={shls:?}");
        assert_within_tolerance(
            &wrapper_scalars,
            &raw_scalars,
            &format!("raw spinor nuclear shell interpretation shls={shls:?}"),
        );
    }
}
...
fn spinor_raw_is_optimizer_invariant_against_wrapper() {
    let (atm, bas, env) = stable_raw_layout();
    let baseline_options = phase3_optimizer_options(&["one-e-nuclear-spinor-optimizer-off"]);
    let optimized_options = phase3_optimizer_options(&["one-e-nuclear-spinor-optimizer-on"]);
    ...
    assert_within_tolerance(
        &baseline_output,
        &optimized_output,
        "raw spinor nuclear optimizer on/off parity",
    );
    assert_within_tolerance(
        &wrapper_scalars,
        &baseline_output,
        "raw spinor nuclear optimizer-off vs wrapper",
    );
    assert_within_tolerance(
        &wrapper_scalars,
        &optimized_output,
        "raw spinor nuclear optimizer-on vs wrapper",
    );
}
...
fn evaluate_raw_spherical_nuclear(
    atm: &[i32],
    bas: &[i32],
    env: &[f64],
    shls: &[i32],
    ...
    .unwrap_or_else(|err| panic!("raw spherical nuclear query failed for {shls:?}: {err:?}"));
    ...
    .unwrap_or_else(|err| panic!("raw spherical nuclear evaluate failed for {shls:?}: {err:?}"));

    (workspace, output)
}

fn evaluate_raw_spinor_nuclear(
    atm: &[i32],
    bas: &[i32],
    env: &[f64],
    shls: &[i32],
    ...
    .unwrap_or_else(|err| panic!("raw spinor nuclear query failed for {shls:?}: {err:?}"));
    ...
    .unwrap_or_else(|err| panic!("raw spinor nuclear evaluate failed for {shls:?}: {err:?}"));

    (workspace, output)
}

fn wrapper_nuclear_spherical(
    atm: &[i32],
    bas: &[i32],
    env: &[f64],
    shls: &[i32],
    ...
    cint.integrate("int1e_nuc", None, shls_slice).into()
}

fn wrapper_nuclear_spinor(
    atm: &[i32],
    bas: &[i32],
    env: &[f64],
    shls: &[i32],
    ...
    let (out, dims) = cint.integrate_spinor("int1e_nuc", None, shls_slice).into();
...
fn flatten_real_output(output: EvaluationOutput) -> Vec<f64> {
    match output {
        ...
        EvaluationOutput::Spinor(values) => panic!(
            "spherical nuclear must produce real output, got spinor len={}",
            values.len()
        ),
    }
}
...
fn flatten_spinor_output(output: EvaluationOutput) -> Vec<f64> {
    match output {
        ...
        EvaluationOutput::Real(values) => {
            panic!(
                "spinor nuclear must produce complex output, got real len={}",
                values.len()
            )
        }
    }
