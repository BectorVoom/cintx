...
#[test]
fn safe_api_matches_wrapper_for_reduced_one_e_kinetic_cartesian_case() {
    let basis = stable_safe_basis();
    let (atm, bas, env) = stable_raw_layout();
    let options = parity_options(&["one-e-kinetic-cartesian-safe-wrapper"]);
    ...
    .expect("safe evaluate must succeed for reduced Cartesian kinetic fixture");
    let safe_scalars = flatten_real_output(safe_tensor.output);

    let (wrapper_scalars, wrapper_dims) =
        wrapper_kinetic_cartesian(&atm, &bas, &env, REDUCED_D_S_RAW_SHLS);
    ...
    assert_within_tolerance(
        &wrapper_scalars,
        &safe_scalars,
        "safe Cartesian kinetic vs wrapper reduced d/s case",
    );
}

#[test]
fn safe_evaluate_into_matches_wrapper_for_reduced_one_e_kinetic_cartesian_case() {
    let basis = stable_safe_basis();
    let (atm, bas, env) = stable_raw_layout();
    let options = parity_options(&["one-e-kinetic-cartesian-safe-evaluate-into-wrapper"]);
    ...
    .expect("safe evaluate_into must succeed for reduced Cartesian kinetic fixture");
    ...
    assert_within_tolerance(
        &wrapper_scalars,
        &safe_output,
        "safe evaluate_into Cartesian kinetic vs wrapper reduced d/s case",
    );
}

#[test]
fn raw_api_matches_wrapper_for_reduced_one_e_kinetic_cartesian_case() {
    let (atm, bas, env) = stable_raw_layout();
    let options = parity_options(&["one-e-kinetic-cartesian-raw-wrapper"]);

    let (workspace, raw_scalars) =
        evaluate_raw_cartesian_kinetic(&atm, &bas, &env, REDUCED_D_S_RAW_SHLS, &options);
    let (wrapper_scalars, wrapper_dims) =
        wrapper_kinetic_cartesian(&atm, &bas, &env, REDUCED_D_S_RAW_SHLS);
    ...
    assert_within_tolerance(
        &wrapper_scalars,
        &raw_scalars,
        "raw Cartesian kinetic vs wrapper reduced d/s case",
    );
}

#[test]
fn raw_api_uses_libcint_flat_layout_for_cartesian_kinetic_output() {
    let (atm, bas, env) = stable_raw_layout();
    let options = parity_options(&["one-e-kinetic-cartesian-layout"]);

    let (workspace, raw_scalars) =
        evaluate_raw_cartesian_kinetic(&atm, &bas, &env, LAYOUT_D_P_RAW_SHLS, &options);
    let (wrapper_scalars, wrapper_dims) =
        wrapper_kinetic_cartesian(&atm, &bas, &env, LAYOUT_D_P_RAW_SHLS);
    ...
    assert_within_tolerance(
        &wrapper_scalars,
        &raw_scalars,
        "raw Cartesian kinetic must match libcint flat column-major layout",
    );
...
#[test]
fn raw_cartesian_kinetic_interprets_atm_bas_env_and_shls_like_wrapper() {
    let (atm, bas, env) = stable_raw_layout();
    let options = parity_options(&["one-e-kinetic-cartesian-raw-inputs"]);
    ...
    for &shls in &[
        ...
        let (workspace, raw_scalars) =
            evaluate_raw_cartesian_kinetic(&atm, &bas, &env, shls, &options);
        let (wrapper_scalars, wrapper_dims) = wrapper_kinetic_cartesian(&atm, &bas, &env, shls);

        assert_eq!(workspace.dims, wrapper_dims, "dims drift for shls={shls:?}");
        assert_within_tolerance(
            &wrapper_scalars,
            &raw_scalars,
            &format!("raw Cartesian kinetic shell interpretation shls={shls:?}"),
        );
    }
}

#[test]
fn raw_cartesian_kinetic_is_optimizer_invariant_against_wrapper() {
    let (atm, bas, env) = stable_raw_layout();
    let baseline_options = phase3_optimizer_options(&["one-e-kinetic-cartesian-optimizer-off"]);
    let optimized_options = phase3_optimizer_options(&["one-e-kinetic-cartesian-optimizer-on"]);
    ...
    .expect("baseline raw query must succeed for reduced Cartesian kinetic fixture");
    ...
    .expect("optimized raw query must succeed for reduced Cartesian kinetic fixture");
    ...
    .expect("baseline raw evaluate must succeed for reduced Cartesian kinetic fixture");
    raw::evaluate_compat(
        kinetic_operator(),
        Representation::Cartesian,
        &optimized_workspace,
    ...
    .expect("optimized raw evaluate must succeed for reduced Cartesian kinetic fixture");
    ...
    assert_within_tolerance(
        &baseline_output,
        &optimized_output,
        "raw Cartesian kinetic optimizer on/off parity for reduced d/s case",
    );
    assert_within_tolerance(
        &wrapper_scalars,
        &baseline_output,
        "raw Cartesian kinetic baseline optimizer-off vs wrapper reduced d/s case",
    );
    assert_within_tolerance(
        &wrapper_scalars,
        &optimized_output,
        "raw Cartesian kinetic optimizer-on vs wrapper reduced d/s case",
    );
}
...
fn evaluate_raw_cartesian_kinetic(
    atm: &[i32],
    bas: &[i32],
    env: &[f64],
    shls: &[i32],
    ...
    .unwrap_or_else(|err| panic!("raw query failed for Cartesian kinetic {shls:?}: {err:?}"));
    ...
    .unwrap_or_else(|err| panic!("raw evaluate failed for Cartesian kinetic {shls:?}: {err:?}"));

    (workspace, output)
}

fn wrapper_kinetic_cartesian(
    atm: &[i32],
    bas: &[i32],
    env: &[f64],
    shls: &[i32],
    ...
    let (scalars, dims) = cint.integrate("int1e_kin", None, shls_slice).into();
    (scalars, dims)
}
...
fn flatten_real_output(output: EvaluationOutput) -> Vec<f64> {
    match output {
        ...
        EvaluationOutput::Spinor(values) => {
            panic!(
                "Cartesian kinetic must produce real output, got spinor len={}",
                values.len()
            )
        }
    }
...
fn col_major_to_row_major(column_major: &[f64], dims: &[usize]) -> Vec<f64> {
    assert_eq!(dims.len(), 2, "1e kinetic fixture must be rank-2");
    let di = dims[0];
    let dj = dims[1];
