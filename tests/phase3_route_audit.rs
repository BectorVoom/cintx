...
use cintx::{
    raw, resolve_capi_route, resolve_raw_route, resolve_route, resolve_safe_route,
    route_manifest_entries, route_manifest_lock_json, safe, CpuRouteKey, ExecutionRequest,
    IntegralFamily, LibcintRsError, Operator, OperatorKind, RawQueryRequest, Representation,
    RouteStatus, RouteSurface, WorkspaceQueryOptions,
};
...
fn route_coverage_completeness_for_supported_policy_set() {
    let mut expected = stable_phase2_matrix()
        .into_iter()
        .map(|row| row.route_key())
        .collect::<HashSet<_>>();
    expected.insert(CpuRouteKey::new(
    ...
    assert_eq!(implemented.len(), 20);

    for key in expected {
        resolve_route(key, RouteSurface::Safe)
            .unwrap_or_else(|err| panic!("shared resolver unexpectedly rejected {key:?}: {err:?}"));
    }
}
...
fn shell_tuple_for_family(family: IntegralFamily) -> Vec<usize> {
    match family {
        ...
        IntegralFamily::TwoElectron => vec![0usize, 1usize, 2usize, 3usize],
    }
}
