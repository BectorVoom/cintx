...
fn spherical_safe_evaluate_matches_wrapper_for_reduced_one_e_case() {
    ...
    let options = parity_options(&["one-e-overlap-spherical-safe-wrapper"]);
    ...
    .expect("safe spherical overlap evaluate must succeed");
    let safe_scalars = flatten_real_output(safe_tensor.output);

    let (wrapper_scalars, wrapper_dims) =
        wrapper_overlap_spherical(&atm, &bas, &env, REDUCED_D_P_RAW_SHLS);
    ...
    assert_within_tolerance(
        &wrapper_scalars,
        &safe_scalars,
        "safe spherical overlap vs wrapper reduced d/p case",
    );
}
...
fn spherical_safe_evaluate_into_matches_wrapper_for_reduced_one_e_case() {
    let basis = stable_safe_basis();
    let (atm, bas, env) = stable_raw_layout();
    let options = parity_options(&["one-e-overlap-spherical-safe-evaluate-into-wrapper"]);

    let (_, wrapper_dims) = wrapper_overlap_spherical(&atm, &bas, &env, REDUCED_D_P_RAW_SHLS);
    ...
    .expect("safe spherical overlap evaluate_into must succeed");
    ...
    assert_within_tolerance(
        &wrapper_scalars,
        &safe_output,
        "safe spherical overlap evaluate_into vs wrapper reduced d/p case",
    );
}
...
fn spherical_raw_matches_wrapper_for_reduced_one_e_case() {
    let (atm, bas, env) = stable_raw_layout();
    let options = parity_options(&["one-e-overlap-spherical-raw-wrapper"]);

    let (workspace, raw_scalars) =
        evaluate_raw_spherical_overlap(&atm, &bas, &env, REDUCED_D_P_RAW_SHLS, &options);
    let (wrapper_scalars, wrapper_dims) =
        wrapper_overlap_spherical(&atm, &bas, &env, REDUCED_D_P_RAW_SHLS);
    ...
    assert_within_tolerance(
        &wrapper_scalars,
        &raw_scalars,
        "raw spherical overlap vs wrapper reduced d/p case",
    );
}
...
fn spherical_raw_uses_libcint_flat_layout() {
    let (atm, bas, env) = stable_raw_layout();
    let options = parity_options(&["one-e-overlap-spherical-layout"]);
    ...
    assert_within_tolerance(
        &wrapper_scalars,
        &raw_scalars,
        "raw spherical overlap must preserve libcint flat column-major layout",
    );

    let row_major_max_diff = max_abs_diff(&row_major_scalars, &raw_scalars);
    assert!(
        row_major_max_diff > 1e-9,
        "raw spherical overlap unexpectedly aligns with row-major flattening"
    );
}
...
fn spherical_raw_interprets_atm_bas_env_and_shls_like_wrapper() {
    let (atm, bas, env) = stable_raw_layout();
    let options = parity_options(&["one-e-overlap-spherical-raw-inputs"]);

    for &shls in &[&[0, 1][..], REDUCED_D_P_RAW_SHLS, &[2, 2][..], &[3, 2][..]] {
        let (workspace, raw_scalars) =
            evaluate_raw_spherical_overlap(&atm, &bas, &env, shls, &options);
        let (wrapper_scalars, wrapper_dims) = wrapper_overlap_spherical(&atm, &bas, &env, shls);

        assert_eq!(workspace.dims, wrapper_dims, "dims drift for shls={shls:?}");
        assert_within_tolerance(
            &wrapper_scalars,
            &raw_scalars,
            &format!("raw spherical overlap shell interpretation shls={shls:?}"),
        );
    }
}
...
fn spherical_raw_is_optimizer_invariant_against_wrapper() {
    let (atm, bas, env) = stable_raw_layout();
    let baseline_options = phase3_optimizer_options(&["one-e-overlap-spherical-optimizer-off"]);
    let optimized_options = phase3_optimizer_options(&["one-e-overlap-spherical-optimizer-on"]);
    ...
    assert_within_tolerance(
        &baseline_output,
        &optimized_output,
        "raw spherical overlap optimizer on/off parity",
    );
    assert_within_tolerance(
        &wrapper_scalars,
        &baseline_output,
        "raw spherical overlap optimizer-off vs wrapper",
    );
    assert_within_tolerance(
        &wrapper_scalars,
        &optimized_output,
        "raw spherical overlap optimizer-on vs wrapper",
    );
}
...
fn spinor_safe_evaluate_matches_wrapper_for_reduced_one_e_case() {
    let basis = stable_safe_basis();
    let (atm, bas, env) = stable_raw_layout();
    let options = parity_options(&["one-e-overlap-spinor-safe-wrapper"]);
    ...
    .expect("safe spinor overlap evaluate must succeed");
    let safe_scalars = flatten_spinor_output(safe_tensor.output);

    let (wrapper_scalars, wrapper_dims) =
        wrapper_overlap_spinor(&atm, &bas, &env, REDUCED_D_P_RAW_SHLS);
    ...
    assert_within_tolerance(
        &wrapper_scalars,
        &safe_scalars,
        "safe spinor overlap vs wrapper reduced d/p case",
    );
}
...
fn spinor_safe_evaluate_into_matches_wrapper_for_reduced_one_e_case() {
    let basis = stable_safe_basis();
    let (atm, bas, env) = stable_raw_layout();
    let options = parity_options(&["one-e-overlap-spinor-safe-evaluate-into-wrapper"]);

    let (_, wrapper_dims) = wrapper_overlap_spinor(&atm, &bas, &env, REDUCED_D_P_RAW_SHLS);
    let element_count = checked_product(&wrapper_dims);
    ...
    .expect("safe spinor overlap evaluate_into must succeed");
    ...
    assert_within_tolerance(
        &wrapper_scalars,
        &flattened_safe,
        "safe spinor overlap evaluate_into vs wrapper reduced d/p case",
    );
}
...
fn spinor_raw_matches_wrapper_for_reduced_one_e_case() {
    let (atm, bas, env) = stable_raw_layout();
    let options = parity_options(&["one-e-overlap-spinor-raw-wrapper"]);

    let (workspace, raw_scalars) =
        evaluate_raw_spinor_overlap(&atm, &bas, &env, REDUCED_D_P_RAW_SHLS, &options);
    let (wrapper_scalars, wrapper_dims) =
        wrapper_overlap_spinor(&atm, &bas, &env, REDUCED_D_P_RAW_SHLS);
    ...
    assert_within_tolerance(
        &wrapper_scalars,
        &raw_scalars,
        "raw spinor overlap vs wrapper reduced d/p case",
    );
}
...
fn spinor_raw_uses_libcint_flat_layout() {
    let (atm, bas, env) = stable_raw_layout();
    let options = parity_options(&["one-e-overlap-spinor-layout"]);
    ...
    assert_within_tolerance(
        &wrapper_scalars,
        &raw_scalars,
        "raw spinor overlap must preserve libcint flat column-major layout",
    );

    let row_major_max_diff = max_abs_diff(&row_major_scalars, &raw_scalars);
    assert!(
        row_major_max_diff > 1e-9,
        "raw spinor overlap unexpectedly aligns with row-major flattening"
    );
}
...
fn spinor_raw_interprets_atm_bas_env_and_shls_like_wrapper() {
    let (atm, bas, env) = stable_raw_layout();
    let options = parity_options(&["one-e-overlap-spinor-raw-inputs"]);

    for &shls in &[&[0, 1][..], REDUCED_D_P_RAW_SHLS, &[2, 2][..], &[3, 2][..]] {
        let (workspace, raw_scalars) =
            evaluate_raw_spinor_overlap(&atm, &bas, &env, shls, &options);
        let (wrapper_scalars, wrapper_dims) = wrapper_overlap_spinor(&atm, &bas, &env, shls);

        assert_eq!(workspace.dims, wrapper_dims, "dims drift for shls={shls:?}");
        assert_within_tolerance(
            &wrapper_scalars,
            &raw_scalars,
            &format!("raw spinor overlap shell interpretation shls={shls:?}"),
        );
    }
}
...
fn spinor_raw_is_optimizer_invariant_against_wrapper() {
    let (atm, bas, env) = stable_raw_layout();
    let baseline_options = phase3_optimizer_options(&["one-e-overlap-spinor-optimizer-off"]);
    let optimized_options = phase3_optimizer_options(&["one-e-overlap-spinor-optimizer-on"]);
    ...
    assert_within_tolerance(
        &baseline_output,
        &optimized_output,
        "raw spinor overlap optimizer on/off parity",
    );
    assert_within_tolerance(
        &wrapper_scalars,
        &baseline_output,
        "raw spinor overlap optimizer-off vs wrapper",
    );
    assert_within_tolerance(
        &wrapper_scalars,
        &optimized_output,
        "raw spinor overlap optimizer-on vs wrapper",
    );
}

fn overlap_operator() -> Operator {
    Operator::new(IntegralFamily::OneElectron, OperatorKind::Overlap)
        .expect("one-electron overlap operator must be valid")
}
...
fn evaluate_raw_spherical_overlap(
    atm: &[i32],
    bas: &[i32],
    env: &[f64],
    shls: &[i32],
    ...
    .unwrap_or_else(|err| panic!("raw spherical overlap query failed for {shls:?}: {err:?}"));
    ...
    .unwrap_or_else(|err| panic!("raw spherical overlap evaluate failed for {shls:?}: {err:?}"));

    (workspace, output)
}

fn evaluate_raw_spinor_overlap(
    atm: &[i32],
    bas: &[i32],
    env: &[f64],
    shls: &[i32],
    ...
    .unwrap_or_else(|err| panic!("raw spinor overlap query failed for {shls:?}: {err:?}"));
    ...
    .unwrap_or_else(|err| panic!("raw spinor overlap evaluate failed for {shls:?}: {err:?}"));

    (workspace, output)
}

fn wrapper_overlap_spherical(
    atm: &[i32],
    bas: &[i32],
    env: &[f64],
    shls: &[i32],
    ...
    cint.integrate("int1e_ovlp", None, shls_slice).into()
}

fn wrapper_overlap_spinor(
    atm: &[i32],
    bas: &[i32],
    env: &[f64],
    shls: &[i32],
    ...
    let (out, dims) = cint.integrate_spinor("int1e_ovlp", None, shls_slice).into();
...
fn flatten_real_output(output: EvaluationOutput) -> Vec<f64> {
    match output {
        ...
        EvaluationOutput::Spinor(values) => panic!(
            "spherical overlap must produce real output, got spinor len={}",
            values.len()
        ),
    }
}
...
fn flatten_spinor_output(output: EvaluationOutput) -> Vec<f64> {
    match output {
        ...
        EvaluationOutput::Real(values) => {
            panic!(
                "spinor overlap must produce complex output, got real len={}",
                values.len()
            )
        }
    }
