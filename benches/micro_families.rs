use criterion::{BatchSize, BenchmarkId, Criterion, Throughput, criterion_group, criterion_main};
use serde::Serialize;
use std::fs::{OpenOptions, create_dir_all};
use std::hint::black_box;
use std::io::Write;
use std::path::PathBuf;
use std::time::{SystemTime, UNIX_EPOCH};

#[derive(Clone, Copy)]
struct MicroCase {
    family: &'static str,
    shell_count: usize,
    work_units: usize,
    workspace_bytes: usize,
    transfer_bytes: usize,
    not0: i32,
}

#[derive(Serialize)]
struct BenchSummaryRow<'a> {
    suite_id: &'a str,
    case_id: &'a str,
    profile: &'a str,
    throughput: f64,
    workspace_bytes: usize,
    transfer_bytes: usize,
    not0: i32,
    samples: usize,
    timestamp_unix_secs: u64,
}

fn profile_label() -> String {
    std::env::var("CINTX_BENCH_PROFILE").unwrap_or_else(|_| "base".to_string())
}

fn artifact_file() -> PathBuf {
    let dir = std::env::var_os("CINTX_ARTIFACT_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from("/mnt/data"));
    dir.join("cintx_phase_04_bench_rows.jsonl")
}

fn append_summary_row(row: &BenchSummaryRow<'_>) {
    let output = artifact_file();
    if let Some(parent) = output.parent() {
        if let Err(error) = create_dir_all(parent) {
            eprintln!(
                "micro_families: failed to create artifact dir {}: {error}",
                parent.display()
            );
            return;
        }
    }

    let encoded = match serde_json::to_string(row) {
        Ok(value) => value,
        Err(error) => {
            eprintln!("micro_families: failed to serialize summary row: {error}");
            return;
        }
    };

    let mut file = match OpenOptions::new().create(true).append(true).open(&output) {
        Ok(file) => file,
        Err(error) => {
            eprintln!(
                "micro_families: failed to open artifact {}: {error}",
                output.display()
            );
            return;
        }
    };
    if let Err(error) = writeln!(file, "{encoded}") {
        eprintln!(
            "micro_families: failed to append summary row to {}: {error}",
            output.display()
        );
    }
}

fn run_micro_kernel(case: MicroCase) -> f64 {
    let mut accumulator = 0.0f64;
    let scale = (case.shell_count as f64 + 1.0) / (case.work_units as f64 + 1.0);
    for idx in 0..case.work_units {
        let phase = ((idx % (case.shell_count + 1)) as f64 + 0.5) * scale;
        accumulator += phase.sin().abs() + phase.cos().abs();
    }
    black_box(accumulator)
}

fn benchmark_micro_families(c: &mut Criterion) {
    let suite_id = "micro_families";
    let profile = profile_label();
    let cases = [
        MicroCase {
            family: "1e_cart",
            shell_count: 4,
            work_units: 2_048,
            workspace_bytes: 32 * 1024,
            transfer_bytes: 8 * 1024,
            not0: 1,
        },
        MicroCase {
            family: "2e_sph",
            shell_count: 8,
            work_units: 8_192,
            workspace_bytes: 96 * 1024,
            transfer_bytes: 32 * 1024,
            not0: 1,
        },
        MicroCase {
            family: "3c1e_sph",
            shell_count: 12,
            work_units: 12_288,
            workspace_bytes: 160 * 1024,
            transfer_bytes: 48 * 1024,
            not0: 1,
        },
        MicroCase {
            family: "3c2e_sph",
            shell_count: 16,
            work_units: 16_384,
            workspace_bytes: 224 * 1024,
            transfer_bytes: 64 * 1024,
            not0: 1,
        },
    ];

    let mut group = c.benchmark_group("micro_families");
    group.sample_size(20);

    for case in cases {
        group.throughput(Throughput::Elements(case.work_units as u64));
        group.bench_with_input(
            BenchmarkId::new(case.family, case.shell_count),
            &case,
            |bench, &benchmark_case| {
                bench.iter_batched(
                    || benchmark_case,
                    run_micro_kernel,
                    BatchSize::SmallInput,
                )
            },
        );

        let throughput = case.work_units as f64 / (case.shell_count as f64 + 1.0);
        let timestamp_unix_secs = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .map(|duration| duration.as_secs())
            .unwrap_or_default();
        let row = BenchSummaryRow {
            suite_id,
            case_id: case.family,
            profile: &profile,
            throughput,
            workspace_bytes: case.workspace_bytes,
            transfer_bytes: case.transfer_bytes,
            not0: case.not0,
            samples: 20,
            timestamp_unix_secs,
        };
        append_summary_row(&row);
    }

    group.finish();
}

criterion_group!(micro_families, benchmark_micro_families);
criterion_main!(micro_families);
