...
- Output path: `/mnt/data/libcint_detailed_design_resolved_en.md`
...
| `/mnt/data/libcint_rust_api_manifest.csv` | Manifest snapshot at design time |
...
3. The GPU backend assumes CubeCL. This is a design constraint and is not part of upstream libcint itself.
...
|---|---|---|
| A1 | The library has three layers: Safe Rust API, raw compatibility API, and optional C ABI shim | To balance Rust usability and libcint migration compatibility |
...
| 4c1e bug envelope | The accepted range for 4c1e is limited to **cart/sph, scalar, natural dims, max(l)<=4, CPU backend, and oracle+identity test pass**; all other cases are treated as `UnsupportedApi` | Execution conditions for 4c1e, tests, release gate | 3.11.2, 12.5, 13.4, 16.4 |
...
### 1.1 Objective
This design document defines an **implementation-ready detailed design** for redesigning and reimplementing the API families provided by libcint in Rust. The primary goals are as follows.

1. Achieve **near-equivalent API results** to libcint
...
- CPU backend and CubeCL GPU backend
- Raw compatibility API, Safe Rust API, and optional C ABI shim

### 1.3 Out of Scope
- Matching libcint’s internal implementation
...
### 1.5 libcint Compatibility Policy
...
|---|---|---|
| FR-01 | Cover all libcint APIs | Includes base APIs, helpers, optimizers, legacy wrappers, and source-only / optional families |
...
| FR-07 | Provide a single logical API across CPU and GPU (CubeCL) | Shared planner, separated backends |
| FR-08 | Support batching / chunking / memory limits | Balance safe stop with throughput for large-scale evaluation |
| FR-09 | Provide helper / transform / optimizer APIs | Compatibility scope includes not only the integral core but also libcint utility APIs |

### 2.2 Non-Functional Requirements
| ID | Requirement | Design mapping |
|---|---|---|
| NFR-01 | High speed | Shell bucketing, screening, SIMD, Rayon, CubeCL specialization |
...
|---|---|---|
| CR-01 | API results are nearly equivalent to libcint | Oracle comparison |
...
- The planner shall have CPU/GPU crossover heuristics
- It shall evaluate both throughput per batch and peak memory
- CPU is preferred for small problems, GPU for large homogeneous batches
- The baseline is fixed after the initial implementation, but **continuous libcint comparison benchmarking** is mandatory
- The planner shall emit dispatch reasons through `tracing`
...
- Public: `Result<T, LibcintRsError>` (`thiserror v2`)
...
- Oracle comparison, property tests, failure-path tests, OOM tests, and CPU/GPU consistency tests
- Tolerances are defined per operator category
- Helper / legacy / transform APIs are also included in comparison coverage

---

## 3. Organization of libcint Specifications

### 3.1 API Layers Provided by libcint
...
The design-time inventory is based on `/mnt/data/libcint_rust_api_manifest.csv`.
...
### 3.3 Manifest Schema
The generated manifest in `libcint-ops` must contain at least the following fields.
...
- The component axis follows libcint’s family-specific ordering
- The Safe API returns logical shapes, but the compat writer writes into libcint’s flat layout
...
- **Permanent remedy**: if `Validated4C1E` is expanded, the same PR must add oracle comparison, identity tests, random fuzzing over 10,000 cases, and CPU/GPU consistency for the added region
- **Responsibility split**:
  - `runtime::planner`: classifier implementation
  - `cpu::center_4c1e`: execute only `Validated4C1E`
...
- Do not add `with-gtg` to the `feature_flag` of the `libcint-ops` manifest
- Do not expose GTG entry points in `libcint-compat`, `libcint-capi`, or the facade
...
|---|---|---|
| L1: Result Conformance | Output matches libcint within tolerance | Required |
...
| L4: Internal Layout Conformance | Internal scratch matches libcint | Out of scope |
...
- `libcint-rs`: facade
- `libcint-core`: domain / error / tensor / traits
- `libcint-ops`: generated manifest
- `libcint-runtime`: planner / scheduler / workspace / tracing
- `libcint-cpu`: CPU kernels
- `libcint-cubecl`: GPU kernels
- `libcint-compat`: raw / helper / legacy
- `libcint-capi`: optional C ABI shim
- `libcint-oracle`: dev-only oracle
...
- The C ABI shim stores `LibcintRsErrorReport` in thread-local storage

### 4.12 Unsafe Code Policy
| Area | `unsafe` allowed? | Condition |
|---|---|---|
| Accepting raw pointers | Yes | Compat / C ABI only |
| SIMD intrinsics | Yes | Restricted to the CPU backend internals |
...
| `runtime::dispatch` | CPU/GPU auto dispatch | Internal |
| `cpu::*` | CPU kernels / SIMD / transforms | Internal |
| `cubecl::*` | GPU kernels / transfers / fallback | Internal |
| `oracle::*` | libcint comparison harness | Dev-only |
...
pub trait WorkspaceAllocator {
    type Buffer<T>;
    fn try_alloc_uninit<T>(&self, elems: usize, align: usize) -> Result<Self::Buffer<T>, LibcintRsError>;
...
) -> Result<ExecutionStats, LibcintRsError>;
```
...
- CPU runs chunks in parallel using Rayon
- Optimizer caches are shared read-only
- Workspaces are checked out thread-locally
- GPU serializes per stream/queue as needed
...
- libcint has a uniform API and minimal precomputation (`README.rst:38-60`). To reproduce this in Rust, the most consistent approach is a design that separates safe APIs while retaining raw compatibility.
- Supporting both CPU and GPU requires a shared planner; having a separate runtime per family would make maintenance costs excessively high.
...
- Upstream libcint also assumes optimizer use, BLAS, and low memory usage (`README.rst:54-70`)
...
- Grouping batches in the planner increases occupancy for both CPU and GPU

### 9.6 Basis for Memory-Efficiency Strategy
- libcint is designed to keep intermediates small (`README.rst:66-70`), so direct writes and workspace reuse are the consistent approach in a reimplementation as well
- Treating memory limits and chunking as first-class concepts enables safe-stop behavior even for huge inputs

### 9.7 Why Not Match libcint’s Internal Implementation
...
|---|---|
| Near-literal Rust port of libcint C implementation | Inconsistent with Rust-like APIs, OOM-safe stop behavior, and the GPU backend |
...
| `rayon` | Adopt | CPU parallelism | Allows concise chunk-level parallelism | A custom thread pool is excessive |
...
| `bindgen` | Dev-only adopt | Generate oracle FFI | Auto-binds the libcint oracle | Handwritten FFI is a maintenance burden |
| `cc` | Dev-only adopt | Vendored libcint build | Enables a hermetic oracle harness | Avoids external environment dependencies |
...
| `oracle` | Enable the vendored libcint comparison harness |
...
3. SoA metadata
4. CPU SIMD dispatch (AVX2/AVX512/NEON)
5. Chunk parallelism via Rayon
...
### 12.5 CPU/GPU Auto Dispatch
```text
if feature disabled -> CPU
else if unsupported family/rep -> CPU fallback
else if estimated transfer_cost > kernel_gain -> CPU
else if batch_size * flop_density >= threshold -> GPU
else CPU
```
The initial conservative heuristic is as follows.
- `batch_size < 8` is generally CPU
- Output smaller than 64 Ki elements is generally CPU
- `spinor` or `unstable_source` families are CPU-preferred
- `grids`, `3c2e`, and large homogeneous 2e are GPU-preferred candidates
...
### 13.4 Result Comparison Tests Against libcint
- Treat vendored libcint as the oracle
...
| Implementation 2 | CPU backend (1e/2e/2c2e) | Oracle comparison passes | Layout mismatch |
| Implementation 3 | 3c1e/3c2e + optimizer | Oracle comparison passes | Transform complexity |
| Implementation 4 | CubeCL backend | CPU/GPU consistency passes | Transfer overhead |
...
│   │       ├── dispatch.rs             # CPU/GPU auto dispatch
...
│   │       ├── lib.rs
│   │       ├── kernels/
│   │       │   ├── one_electron.rs     # 1e kernels
│   │       │   ├── two_electron.rs     # 2e kernels
│   │       │   ├── center_2c2e.rs      # 2c2e kernels
│   │       │   ├── center_3c1e.rs      # 3c1e kernels
│   │       │   ├── center_3c2e.rs      # 3c2e kernels
│   │       │   └── center_4c1e.rs      # 4c1e kernels
│   │       ├── transform/
│   │       │   ├── c2s.rs              # cart→sph
│   │       │   └── c2spinor.rs         # cart→spinor
│   │       ├── screening.rs            # Pair screening
│   │       ├── simd.rs                 # Runtime SIMD dispatch
│   │       └── executor.rs             # `BackendExecutor` implementation
│   ├── libcint-cubecl/
│   │   ├── Cargo.toml
│   │   └── src/
│   │       ├── lib.rs
...
│   └── libcint-oracle/
│   │       ├── Cargo.toml
│   │       ├── build.rs                # Vendored libcint build + bindgen
│   │       └── src/
...
│   └── crossover_cpu_gpu.rs            # Dispatch-threshold benchmark
└── ci/
...
### 16.1 Comparison Strategy Using libcint as the Oracle
- Build vendored libcint in the `libcint-oracle` crate and generate wrappers with `bindgen`
...
|---|---|---|---|---|
| R-01 | Numerical | Instability for high-l / many-roots cases | Hard to achieve parity for some families | Maintain a CPU reference path and family-specific tolerances |
...
| R-05 | Implementation | Difficulty of GPU implementation for complex spinor transforms | Reduced GPU coverage | Explicit CPU fallback |
...
2. To expand `Validated4C1E`, pass oracle comparison for the added region, identity tests, 10,000 random fuzz cases, and CPU/GPU consistency in the same PR
